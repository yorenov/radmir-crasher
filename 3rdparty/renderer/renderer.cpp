...
void cRender::DrawQuad( int16_t x1, int16_t y1,
                        int16_t x2, int16_t y2,
                        int16_t x3, int16_t y3,
                        int16_t x4, int16_t y4,
                        RenderDrawType flags,
                        color_t color1, color_t color2,
                        color_t color3, color_t color4 )
{
    if ( !( flags & RenderDrawType_Gradient ) )
    {
        color2 = color1;
    ...
    pVertex[ 1 ] = Vertex_t( x2, y2, color2 );
    pVertex[ 2 ] = Vertex_t( x3, y3, color3 );
    pVertex[ 3 ] = Vertex_t( x4, y4, color4 );
    pVertex[ 4 ] = pVertex[ 0 ];

    m_pDevice->DrawPrimitiveUP(
        ( flags & RenderDrawType_Outlined ) ? D3DPT_LINESTRIP :
        ( flags & RenderDrawType_Filled ) ? D3DPT_TRIANGLEFAN :
        D3DPT_POINTLIST, 4, pVertex, sizeof( Vertex_t ) );
}