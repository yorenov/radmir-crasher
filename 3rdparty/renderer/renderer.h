...
public:
    ...
    void DrawQuad( int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t x3, int16_t y3, int16_t x4, int16_t y4, RenderDrawType flags, color_t color1, color_t color2, color_t color3, color_t color4 );

    //frames per second
    int16_t GetFramerate() const
    {
        return m_iFramerate;
    }
    //milliseconds
    void SetFramerateUpdateRate( const uint16_t iUpdateRate )
    {
        m_iFramerateUpdateRate = iUpdateRate;
    }
